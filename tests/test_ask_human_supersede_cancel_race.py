...
under the same (conv, sender) key, Q1's shielded cancellation cleanup must
remove only its OWN entry, never the live Q2 entry that superseded it."""
import asyncio
import pytest
...
async def test_superseded_asker_cleanup_does_not_remove_live_entry(cfg, logger):
	...
	req1 = registry.get((conv_id, _SENDER)).request_id
	...
	rec = registry.get((conv_id, _SENDER))
