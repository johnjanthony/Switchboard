...
**Known limitation, accepted.** A fully idle at-desk agent sees its queued notice only at its next prompt, because no mechanism exists to wake an idle session (§15). In away mode the state does not arise, since the Stop hook forces every turn to end on a blocking ask.
