...
- **Do not park the loop behind John's replies** (John's direction, 2026-07-30). Ending every turn on a blocking `ask_human` stalls the relay: task notifications queue behind John's tap, and the orchestrator idles until he answers. Two mechanisms compose to fix it:
  ...
  - *John's interject channel is message injection.* John types into the conversation from his phone or Operator whenever he likes; nothing needs to hold a slot open for it. The message reaches the design tier as the resolution of its live blocking `ask_human` (prefixed `[John interjected - not a direct answer to your question]`), a `message_and_await_agent` wake, or a `John (from phone):` notice injected mid-turn or at a turn boundary. Shipped as T-164; `skills/switchboard/SKILL.md` ("John can message you unprompted") is the reference wording. *(Historical note: before T-164 an interim workaround spawned a tiny haiku "parking attendant" agent to hold a standing `ask_human` as a proxy interject channel. Message injection retires it - do not spawn one.)*
  - *The design tier's own non-stalling question is `ask_human(background=true)`.* It returns `{"status": "pending", ...}` immediately and John's answer arrives later through the same delivery ladder, so a question no longer parks the relay. It is NOT John's interject channel, and a pending background ask does NOT satisfy the away-mode turn-end hook - keep using the TaskOutput pattern above to hold a turn open, or a direct blocking `ask_human` to end one. Status still flows one-way via `notify_human`.
