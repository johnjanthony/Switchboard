...
- **MCP:** `mcp_config.json` (workspace `<ws>/.agents/` — verified; global `~/.gemini/config/` — load VERIFIED 2026-07-16 via V2) with `{"mcpServers": {"switchboard": {"serverUrl": "http://localhost:9876/mcp"}}}`. Verified end-to-end against the live Switchboard service: tool discovery, call, and the server's `cli_session_id required` rejection all round-tripped. **As-built caveat (2026-07-16):** agy headless `-p` mode auto-denies MCP tool calls — it needs a `permissions.allow` rule or `--dangerously-skip-permissions` (headless cannot prompt for the `mcp` permission), so the headless round-trip probes used the bypass flag. Interactive agy prompts for the permission normally, so real usage is unaffected. Remote entries accept a `headers` map (Bearer auth for WSL later) per config format; unprobed.
...
- **Hook output contracts** (from the binary's embedded docs, cross-checked with live behavior): `PreToolUse` → `{"decision": "allow"|"deny"|"ask"|"force_ask", "reason"?, "permissionOverrides"?}`; `PostToolUse` → `{}`; `PreInvocation` → `{"injectSteps": [{"ephemeralMessage": ...}|{"userMessage": ...}|{"toolCall": ...}]}` (ephemeral inject live-verified); `PostInvocation` → `{"injectSteps"?, "terminationBehavior": "force_continue"|"terminate"|""}`; `Stop` → `{"decision": "continue", "reason": ...}` blocks the stop and re-enters the loop with the reason injected as a system message (contract documented; live continue-loop VERIFIED 2026-07-16 via V1).
...
| Away-mode turn-end enforcement | Stop hook `block` | Stop hook `continue` + system-message reason (V1 verified 2026-07-16: loop re-enters, model routes to ask_human) |
| Server-restart behavior | MCP tools die (stateful HTTP; known cost) | agy MCP auto-reconnects (V4 verified 2026-07-16; Claude-side auto-reconnect also observed after the V6 restart); startup away-mode auto-clear protects agy sessions identically |
...
**As-built verification (2026-07-16, all PASSED):** the plan executed via subagent-driven development (Tasks 1-7), committed as `9001f73` (plugin 1.5.1), suite 888 green. Live checklist results: V2 (global config load) — agy lists switchboard, cli.log `1 named hooks, 4 total handlers`. Identity round-trip — headless returned `{"status":"ok"}` with no deny-loop (via `--dangerously-skip-permissions`, see §2 caveat); interactively confirmed by an agy peer ("tools appeared without any deny loop; I understand I must pass cli_session_id and cwd inside Arguments"). V6 (`cli` hydration) — antigravity sessions survive a restart still tagged `cli=antigravity`. V1/V3/V4/V5 (away-mode Stop-continue loop, interactive roster parity, restart-reconnect, latency) — all passed. End-to-end — John convened a Claude↔agy conversation; message_and_await_agent round-tripped both directions. The headless `-p` MCP permission gate (§2 caveat) and empty-cwd-in-headless are the only as-built deltas; neither affects interactive real usage.
