...
echo "[$(date -u +%FT%TZ)] agy start path='$WSPATH' session='$SESSION_ID' flag='$SESSION_FLAG' distro=${WSL_DISTRO_NAME:-?} agy=$(command -v agy || echo MISSING) PATH=$PATH" >> "$LOG" 2>&1

cd "$WSPATH"
if [ -z "$PROMPT" ]; then
	agy --add-dir "$WSPATH" --conversation "$SESSION_ID" --dangerously-skip-permissions
else
	agy -i "$PROMPT" --add-dir "$WSPATH" --conversation "$SESSION_ID" --dangerously-skip-permissions
fi
EC=$?
