...
3. **Backgrounded asks:** If the harness moves a blocking `ask_human` to a background task (it does this to MCP calls still running at ~120s), do NOT call `ask_human` again — a re-ask supersedes the live question and loops. End your turn: the live pending question is your handback (the turn-end hook checks for it and allows the turn to end), and John's reply arrives as the background task's result.
4. **Replies:** When `ask_human` returns a reply, do not acknowledge it in the terminal. Treat it as input for your next tool call or task.
5. **Exit:** The **only** exit trigger is John's MOST RECENT prompt explicitly saying he is back ("I'm back", "back at desk"). Call `set_away_mode(false)` as your first action, then resume terminal chat with a concise summary of your work. A back-at-desk message from earlier in the conversation does not retroactively authorize an exit on a later turn. `set_away_mode(false)` also resolves any still-pending `ask_human` questions with the notice `"John is back at his desk; your question was not answered remotely. Re-ask in the terminal."`, so their askers wake in their own terminals; nothing stays parked on the phone.

**At-desk `ask_human` redirect.** If `ask_human` returns the literal string `"ERROR: John is at his desk. Ask this question via the terminal."`, the human is at their desk, and your question has already been delivered to their phone as a passive notification (chat history + Updates channel, no reply UI). Your next action is to **produce the question content verbatim in the terminal** — the human will respond via terminal input. Do NOT retry `ask_human`, do NOT call `set_away_mode(false)` (the flag is already off), and do NOT treat this as an error worth surfacing beyond asking the question. This redirect only applies to `ask_human`; `notify_human` and `send_document_human` always deliver normally regardless of away-mode state. A background ask (`ask_human(..., background=true)`) gets a DIFFERENT at-desk sentinel, `"ERROR: John is at his desk. State your question in the terminal and continue working."` — the two literals are not interchangeable: the blocking sentinel above means ask via the terminal and wait there for the reply, while this one means state the question in the terminal and keep working, do NOT convert a background at-desk return into a blocking terminal ask.

**John can message you unprompted.** John can inject free-form messages into your conversation at any time - not just as a reply to a pending `ask_human`. They arrive one of three ways: (a) as the resolution of a blocked `ask_human`, reading like `[John interjected - not a direct answer to your question] <John's text>` - treat this as a fresh instruction, not an answer, and re-ask your original question afterward if it still needs one; (b) as a `message_and_await_agent` wake whose log contains a line like `John: <John's text>`; or (c) as an injected context note reading like `John (from phone): <John's text>`, delivered mid-turn or at a turn boundary. Act on the content directly - do not treat an interjection as an answer to the question you asked.
...
- **`ask_human(question, sender, title?, format?, suggestions?, background?)`** — blocks until John replies. Returns the reply text; if no reply can arrive it returns one-line JSON: `{"status":"timeout"}` (window elapsed), `{"status":"superseded"}` (a newer ask_human from your own session replaced this call - the newer question is the live one; do not retry this one), or `{"status":"conversation_ended","cause":...}` (conversation ended out from under you). `cause` values: `force-ended` and `merged into target` are terminal - stop, do NOT retry; `combined into <conv-id>; re-ask your question there` means John merged your conversation while your question was pending - you are already a member of the target conversation, so re-ask the same question once. Pass `background=true` for a non-blocking ask: it returns one-line JSON `{"status":"pending","request_id":...}` immediately, and John's answer arrives later — as the resolution of your next blocking `ask_human`, a `message_and_await_agent` wake, or an injected notice. A second background ask from the same session appends to the first card instead of opening a new one: the question text grows with an `Also: <new question>` line, the original `request_id` is kept, and one reply resolves the whole slot. At-desk, `background=true` returns a different sentinel than the blocking form: `"ERROR: John is at his desk. State your question in the terminal and continue working."` — state the question and keep working, do NOT convert it into a blocking terminal ask. Either way, a pending background ask does NOT satisfy the away-mode turn-end hook: an away-mode turn must still end on a blocking `ask_human`. (A blocking ask the harness has moved to a background task DOES satisfy the hook — end the turn, do not re-ask.)
...
- **`message_and_await_agent(sender, message, title?, timeout_seconds?)`** — conversations only. `message` is required and non-empty. Send to peers and block until woken. If you are alone in the conversation your message parks until a peer joins and replies, the wait times out, or John convenes you. `timeout_seconds` caps this wait, clamped server-side to [10s, the server default window]. A `message_and_await_agent` timeout from a `timeout_seconds` you passed is expected loop control - do other work, poll with `join_conversation()`, or re-enter the wait; only a default-window timeout you did not choose triggers the pause-and-record protocol. Returns one-line JSON: `{"status":"ok","log":"...","peers":[...]}` (the conversation delta since your last wake, excluding your own emissions), `{"status":"timeout"}`, `{"status":"superseded"}` (a newer wait from your own session replaced this one — that newer call is the live one), or `{"status":"conversation_ended","cause":...}` (terminal). When called while NOT in any conversation it returns `"ERROR: not in any conversation. End your turn."`.
- **`post_agent_message(sender, message, title?)`** — conversations only. Speak WITHOUT blocking: writes the message, wakes one blocked peer, returns immediately with one-line JSON `{"status":"ok","conversation_id":...,"msg_id":...,"log":"<unseen delta>","peers":[...]}`. Use it to acknowledge a request before starting long work, then deliver the result with `message_and_await_agent`. It never supersedes or disturbs a parked wait from your own session. Same rate-limit bucket as `message_and_await_agent`. A rare `"write_failed": true` on the ok envelope means peers were woken but the phone-history write failed — do not re-post.
- **`join_conversation(sender, ref?, title?)`** — join a conversation. **Never blocks; idempotent.** Pass `ref` (a conversation_id from `lookup_conversation_ids`, a convene notice, or John's prompt) to join that specific conversation — migrating you out of your current one if needed. Omit `ref` and the candidate rule applies: a session already bound to an Active conversation rejoins it; an unbound session lands in the single Active conversation that was itself minted ref-less, still has exactly one alive member, and is younger than ~30 minutes; zero or several such candidates both mint a new room instead. `minted: true` on the reply means you did not find an existing room — you created one. Returns one-line JSON: `{"status":"ok","conversation_id":...,"sender":...,"peers":[...],"log":"...", "minted"?:true, "already_member"?:true}` — `log` is the history you haven't seen (full on first join; unseen delta on re-join), `sender` is your display name after any collision disambiguation. `peers` entries are objects `{sender, state: alive|dormant, waiting, last_spoke_at}`; `waiting=true` means a message sent now will wake that member, not that the peer is idle. To wait for peers after joining, call `message_and_await_agent`.
...
`sender` is **required** on every tool that takes it (`ask_human`, `notify_human`, `send_document_human`, `message_and_await_agent`, `post_agent_message`, `join_conversation`, `leave_conversation`) -- omitting it there raises a schema error, and there is no default. `combine_conversations`, `lookup_conversation_ids`, and `set_away_mode` do not take a `sender`.
...
**Exception:** the literal string `"ERROR: John is at his desk. Ask this question via the terminal."` is NOT a gateway failure — it is the at-desk redirect described in the Away Mode Protocol above. In that specific case, produce the question in the terminal and continue; do not pause. A background ask (`ask_human(..., background=true)`) gets a different at-desk literal, `"ERROR: John is at his desk. State your question in the terminal and continue working."` — also not a failure, but state the question and keep working rather than converting it into a blocking terminal ask.
...
| `superseded` | A newer ask_human or message_and_await_agent from your own session replaced this call | the newer call carries the live question; do not re-ask this one |
...
2. Otherwise `lookup_conversation_ids` — `title_contains` first (titles are agent-set and descriptive), then `sender_contains`. The name John calls an agent is not necessarily its sender string (e.g. "Fable" vs sender "Claude Win"), so on zero hits retry likely variants before concluding it does not exist.
3. Exactly one plausible match: join by ref. Several: pick by `last_activity_at` and member states, or ask John. Zero: ask John.
4. If you went ref-less expecting an existing room and the envelope says `minted: true`, you did NOT find it — you created a new empty room. `leave_conversation` and go back to step 2.
...
# → {"status":"ok","conversation_id":"conv-...","peers":[{"sender":"Claude Win","state":"alive","waiting":true,"last_spoke_at":"..."}],"log":"..."}
...
# → {"status":"ok","conversations":[{"conversation_id":"abc123","title":"Switchboard refactor collab", ...}]}
...
5. **Mid-collab symmetric obligation.** Receiving a message via `message_and_await_agent` passes the live baton to you. You MUST answer with another `message_and_await_agent` call carrying a non-empty `message`. Two failure modes are forbidden:
   ...
   If answering requires substantial work, first `post_agent_message` an ack with an ETA (this does not block you and does not count as passing the baton away), do the work, then reply with `message_and_await_agent`. A wake whose delta is only such a status ack does not pass the baton to you - stay waiting or do something useful.
...
8. If `message_and_await_agent` returns `{"status":"timeout"}` on a default-window wait (one you did not cap with `timeout_seconds`), ping John for a status check (terminal if at-desk, `ask_human` if away-mode). Do not silently abandon peers. A timeout from a `timeout_seconds` value you chose is expected loop control, not this case - see "Waiting without going dark" below.
...
`message_and_await_agent` and `post_agent_message` share a separate per-conversation bucket, but never reject on limit: past the limit they degrade to suppressing the FCM push only - the message still writes and still wakes the peer, John just doesn't get a phone ping for that one. There is no `"ERROR: rate limit exceeded..."` from either tool.
...
3. If it returns the literal string `"ERROR: John is at his desk. Ask this question via the terminal."` (or, if you asked with `background=true`, `"ERROR: John is at his desk. State your question in the terminal and continue working."`), away-mode is off. The summary has already been delivered to John's phone as a passive notification; now repeat it verbatim in the terminal — that's where John is watching.
...
## Waiting without going dark (per-harness)

- **Claude Code:** MCP calls still running after ~120s move to a background task automatically; you keep working and get a completion notification. Backgrounded waits do not survive session exit. To retract a parked wait, TaskStop it - or simply issue a new `message_and_await_agent`, which supersedes the old one (it returns `{"status":"superseded"}`). This is harness behavior, not a switchboard feature - do not assume peers have it.
...
When you are supervising long-running background agents (e.g. an orchestrator executing a plan — see the `background-orchestrator` pattern if your host has it) while away mode is on, the away-mode Stop hook blocks any turn-end unless you hold a live blocking `ask_human`. A question held by ANOTHER agent does not count, and neither does a `background=true` ask. Claude Code's handling of long MCP calls makes this cheap, so supervision needs no turn-holding:

- **Park one blocking ask, then keep supervising.** Call a blocking `ask_human` (e.g. `"Supervising <plan>. Reply here to interject."`). After ~120s the harness moves it to a background task and you keep working, and the hook accepts every turn-end for as long as that ask is pending. Do NOT call `ask_human` again while it is live: a new ask supersedes it. Whenever it resolves (John replied, or it timed out), handle that and park a fresh one before your next turn-end.
- **Background work reaches you on its own.** Task notifications, and background agents' `SendMessage` reports to `main`, are delivered at your next tool call mid-turn, or start a new turn when you are idle. Review, push status to John with `notify_human`, and end the turn. `TaskOutput` no longer exists in the harness, so the old wait-by-blocking pattern is gone. (Mid-turn delivery verified 2026-09-25 at the desk; the whole loop has not yet been run with away mode on.)
- **John's interject channel is message injection, not an attendant.** John interjects from his phone or the Operator dashboard at any moment; no standing agent needs to hold a slot open for it. It reaches you as the resolution of your live blocking `ask_human` (prefixed `[John interjected - not a direct answer to your question]`), a `message_and_await_agent` wake, or a `John (from phone):` notice delivered mid-turn or at a turn boundary — see "John can message you unprompted" above. (Historical note: an interim workaround spawned a tiny haiku agent whose sole job was holding a standing `ask_human` as a proxy interject channel; message injection retires that workaround — do not spawn one.) For your OWN non-stalling question mid-supervision, use `ask_human(..., background=true)` instead: it returns `{"status":"pending",...}` immediately and John's answer still arrives through the same ladder later — but this is NOT John's interject channel, and it does not satisfy the turn-end hook: only the parked blocking ask does.

When the work finishes and John is still away, replace the parked question with the real one: a new blocking `ask_human` (`"Done: <summary>. What's next?"`) supersedes it, which is what you want at that point. Escalations that genuinely need John mid-run also use a direct `ask_human`, which likewise supersedes the parked one and then serves as the parked ask until he answers — message injection is John's channel for interjecting into your work, not yours for asking him something; keep your own questions on `ask_human` (blocking or `background=true`). And expect spawns/resumes to auto-enable away mode as a side effect: if John is actually at the desk during a spawn-driven test session, reset it (with his blessing) or your next turn-end will be blocked.
